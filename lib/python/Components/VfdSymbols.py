...
from enigma import eDBoxLCD, eTimer, iPlayableService, iServiceInformation, pNavigation
...
class SymbolsCheckPoller:
	...
	def Recording(self):
		if fileExists("/proc/stb/lcd/symbol_circle"):
			recordings = len(NavigationInstance.instance.getRecordings(False,Components.RecordingConfig.recType(config.recording.show_rec_symbol_for_rec_types.value)))
			if recordings > 0:
