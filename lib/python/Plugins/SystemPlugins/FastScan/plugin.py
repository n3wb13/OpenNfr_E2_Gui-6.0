...
class FastScanScreen(ConfigListScreen, Screen):
    ...
    def __init__(self, session, nimList):
        ...
        self.transponders = ((12515000,
          ...
          eDVBFrontendParametersSatellite.Pilot_Off), (12070000,
          27500000,
        ...
        self['actions'] = ActionMap(['SetupActions', 'MenuActions'], {'ok': self.keyGo,
         'save': self.keyGo,
    ...
    def readXML(self, xml):
        ...
        try:
            ...
            for lista in tr_list:
                ...
                self.rolloff = 0
                self.pilot = 2
                self.addSatTransponder(tlist, int(self.frequency), int(self.symbolrate), int(self.polarization), int(fec), int(self.inversion), int(orbpos), int(self.system), int(self.modulation), int(self.rolloff), int(self.pilot))

            self.session.openWithCallback(self.bouqmake, ServiceScan, [{'transponders': tlist,
    ...
    def keyGo(self):
        ...
        if prov == 'sky_hd' or prov == 'sky_sd' or prov == 'orf' or prov == 'deutsch_hd' or prov == 'deutsch_sd' or prov == 'sky_orf_de':
            if self.scan_alternative_number_mode.value == True:
                ...
                config.usage.alternative_number_mode.save()
            config.misc.fastscan.last_configuration.value = `(self.scan_nims.value,
