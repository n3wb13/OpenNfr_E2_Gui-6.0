...
class TnAflixGenreScreen(MPScreen):
	...
	def __init__(self, session, mode):
		...
		if self.mode == "empflix":
			...
			self.baseurl = "http://www.empflix.com"
	...
	def genreData(self, data):
		...
		if Cats:
			...
			for (dummy, Url, dummy, Title, dummy) in Cats:
				if not Title == "All":
					Url = self.baseurl + Url
					if self.mode == "tnaflix":
						Url = Url.replace("/featured/", "/most-recent/") + "&page="
					elif self.mode == "moviefap":
						Url = Url + "mr/"
...
class TnAflixFilmScreen(MPScreen, ThumbsHelper):
	...
	def loadData(self, data):
		...
		elif re.match(".*?tnaflix", self.baseurl):
			self.getLastPage(data, 'class="navigation\sclear">(.*?)</div>')
			Movies = re.findall('data-vid=.*?data-original=\'(.*?)\'.*?videoDuration\'>(.*?)<.*?\s+href=\'(.*?)\'\s+title=\'\'>(.*?)<', data, re.S)
		else:
		...
		if Movies:
			if re.match(".*?moviefap", self.baseurl):
				...
				for (Image, Runtime, Url, Title) in Movies:
					Url = self.baseurl + Url
					self.filmliste.append((decodeHtml(Title), Url, Image, Runtime))
			else:
	...
	def getVideoPage(self, data):
		if re.match(".*?tnaflix", self.baseurl):
			url = re.findall('"contentUrl" content=".*?//(.*?)"', data, re.S)
		else:
