...
def CreateAuthoringXML_singleset(job):
	...
	authorxml.append(' <dvdauthor dest="' + (job.workspace+"/dvd") + '" format="' + job.project.menutemplate.settings.video_format.value + '">\n')
	authorxml.append('  <vmgm>\n')
	authorxml.append('   <menus lang="' + iso639language.get_dvd_id(job.project.menutemplate.settings.menulang.value) + '">\n')
...
def CreateAuthoringXML_multiset(job):
	...
	authorxml = []
	authorxml.append('<?xml version="1.0" encoding="utf-8"?>\n')
	authorxml.append(' <dvdauthor dest="' + (job.workspace+"/dvd") + '" jumppad="yes" format="' + job.project.menutemplate.settings.video_format.value + '">\n')
	authorxml.append('  <vmgm>\n')
	authorxml.append('   <menus lang="' + iso639language.get_dvd_id(job.project.menutemplate.settings.menulang.value) + '">\n')
	authorxml.append('    <video aspect="4:3"/>\n')
	if mode.startswith("menu"):
	...
	for i in range( nr_titles ):
		...
		for audiotrack in title.properties.audiotracks:
			...
			if active:
				...
				language = iso639language.get_dvd_id(audiotrack.language.value)
